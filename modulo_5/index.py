...
#operar os dados do dashboard
import pandas as pd
import numpy as np

#montar graficos do dashboard
...
df_data = pd.read_csv("C:\\Users\\cikra\\OneDrive\\Área de Trabalho\\__Asimov.Academy\\DashBoards\\modulo_4\\supermarket_sales.csv")
...
def render_page_content(cities, main_variable):
    ...
    fig_city = px.bar(df_city, x="City", y=main_variable, template="plotly_dark")
    fig_gender = px.bar(df_gender, x="Gender", y=main_variable, color="City", barmode='group', template="plotly_dark")
    fig_payment = px.bar(df_payment, y="Payment", x=main_variable, orientation="h", template="plotly_dark")
    fig_income_time = px.bar(df_income_time, y=main_variable, x="Date",template="plotly_dark")
    fig_product_income = px.bar(df_product_income, x=main_variable, y="Product line", color="City", orientation="h", template="plotly_dark")
